...
If that binary is unavailable and you are inside this repo with built artifacts present, use:
...
When docs and live output differ, trust CLI help and `artifacts/tools.json`.
...
Do not put secrets such as `privateKey`, `authPrivateKey`, or API keys in argv when stdin is available. Prefer:
